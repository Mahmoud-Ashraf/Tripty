...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const placesReq = await fetch(`${baseUrl}places?per_page=20&change_language=${locale}${!!long ? `&long=${long}` : ''}${!!lat ? `&lat=${lat}` : ''}&category_id=${categoryId}${!!trendNow ? '&trend_now=1' : ''}${!!text ? `&text=${text}` : ''}`);
        if (!placesReq.ok) {
